...
describe("generateRandomPassword", () => {
  ...
  it('should throw "passwordLength should be greater than 0" error', () => {
    ...
    expect(generateRandomPasswordCaller(0)).toThrowError("passwordLength must be greater than 0.");

    expect(generateRandomPasswordCaller(-5)).toThrowError("passwordLength must be greater than 0.");
  });
  ...
  it('should throw "You must set at least 1 character type" error', () => {
    ...
    expect(() => generateRandomPassword(config)).toThrowError("You must set at least 1 character type.");
  });
