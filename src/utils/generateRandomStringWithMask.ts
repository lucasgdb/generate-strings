...
export function generateRandomStringWithMask(config?: Config) {
   ...
   for (let i = 0; i < mask.length; i += 1) {
      if (mask[i] === upperCaseMask) {
         const randomPosition = getRandomInteger(
            0,
            upperCaseCharacters.length - 1
         );
